...
describe("settlement", () => {
  ...
  it("pays the words the board actually uses", () => {
    ...
    expect(settleRun(won, 20, "grim", 1).xp).toBe(120);
    expect(settleRun(won, 20, "grim", 1).gold).toBe(20);
    expect(settleRun(cost, 20, "fair", 1).xp).toBe(38);
  ...
  it("still understands the old danger words", () => {
    expect(settleRun(won, 10, "hard", 1).xp).toBe(120);
    expect(settleRun(won, 10, "risky", 1).xp).toBe(75);
  });
});
